      ...
      <div className="relative mx-auto max-w-7xl px-6 sm:px-12">
        <div className="grid gap-10 sm:grid-cols-4 sm:gap-8 lg:gap-12">
          <div className="sm:col-span-2">
            <div className="mb-4 flex items-center gap-3">
              ...
              <img src={images.logo} alt="Lilaas" className="h-7 w-auto brightness-110" onError={(e) => { e.currentTarget.style.display = "none"; }} />
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {["DNV", "CRS", "IP66", "CE", "CAN · J1939", "AESS"].map((b) => (
                <span key={b} className="mono rounded-md border border-orange/15 bg-orange/8 px-3 py-1 text-[10px] font-bold tracking-widest text-orange/80">
                  {b}
                </span>
              ))}
            </div>
          ...
          <div>
            <HUDLabel className="mb-4 opacity-70">Navigation</HUDLabel>
            <nav className="flex flex-col gap-2.5">
              {navLinks.map(({ id, label }) => (
                <a key={id} href={`#${id}`} className="group flex w-fit items-center gap-2 text-sm text-secondary/60 transition-colors hover:text-orange">
          ...
          <div>
            <HUDLabel className="mb-4 opacity-70">Contact</HUDLabel>
            ...
            <Link
              href="/#contact"
              className="mt-5 inline-flex items-center gap-2 rounded-full border border-orange/25 bg-orange/8 px-4 py-2 text-xs font-bold text-orange transition-all hover:border-orange/40 hover:bg-orange/15"
            >
              Get in touch →
            </Link>
          </div>
        ...
        <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
          ...
          <p className="flex items-center gap-2 text-[11px] text-secondary/40">
            ...
            <span className="mono inline-flex items-center gap-1.5 rounded-md border border-white/8 bg-white/3 px-2 py-1 font-semibold tracking-widest text-white/70">
